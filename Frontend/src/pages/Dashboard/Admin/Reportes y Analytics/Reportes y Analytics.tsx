...
export const ReportesYAnalytics = () => {
  ...
  const eventAttendanceMetrics = data?.eventAttendanceMetrics || {
    totalEvents: 0,
    totalInvitations: 0,
    ...
    <Box sx={{ 
        ...
        <Card sx={{ 
          ...
          <CardContent sx={{ p: 3 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <Box sx={{ flex: 1 }}>
                <Typography variant="h3" sx={{ fontWeight: 800, mb: 1, color: '#ffffff' }}>
                  {eventAttendanceMetrics.totalEvents.toLocaleString()}
                </Typography>
                <Typography variant="h6" sx={{ fontWeight: 600, mb: 0.5, color: '#ffffff' }}>
                  Eventos Creados
                </Typography>
                <Typography variant="body2" sx={{ opacity: 0.9, color: '#ffffff' }}>
                  {eventAttendanceMetrics.confirmationRate || 0}% tasa de confirmación
                </Typography>
              </Box>
              <Box sx={{ 
                p: 2, 
      ...
      <Box sx={{ display: 'flex', gap: 10, mb: 10, flexWrap: 'wrap' }}>
       ...
       <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4, mt: 6 }}>
        ...
        {/* Gráficos de Asistencias a Eventos - Nuevo Diseño */}
        <Box sx={{ flex: '1 1 100%', minWidth: 600 }}>
          <Card sx={{ 
            ...powerBICardStyles,
            bgcolor: themeMode === 'dark' ? '#1e293b' : '#ffffff',
            ...
            '&::before': {
              ...
              right: 0,
              height: '4px',
              background: powerBIGradients.info,
            }
          }}>
            <CardContent sx={{ p: 3 }}>
              <Typography variant="h6" sx={{ 
                ...
                📅 Métricas de Asistencias a Eventos
              </Typography>

              {/* Gráficos de Asistencias - Nuevo Diseño */}
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: 4 }}>
                {/* Gráfico de Barras - Estado de Asistencias */}
                <Box>
                  <Typography variant="h6" sx={{ 
                    mb: 3, 
                    fontWeight: 700, 
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={[
                      { 
                        estado: '✅ Confirmadas', 
                        cantidad: eventAttendanceMetrics.confirmedAttendances || 0,
                        color: '#22c55e'
                      },
                      { 
                        estado: '❌ Declinadas', 
                        cantidad: eventAttendanceMetrics.declinedAttendances || 0,
                        color: '#ef4444'
                      },
                      { 
                        estado: '❓ Tal vez', 
                        cantidad: eventAttendanceMetrics.maybeAttendances || 0,
                        color: '#f59e0b'
                      },
                      { 
                        estado: '⏳ Pendientes', 
                        cantidad: eventAttendanceMetrics.pendingAttendances || 0,
                        color: '#6b7280'
                      }
                    ]}>
                      <defs>
                        ...
                        <linearGradient id="maybeGradient" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="#f59e0b" stopOpacity={0.9}/>
                          <stop offset="95%" stopColor="#f59e0b" stopOpacity={0.3}/>
                        </linearGradient>
                      ...
                      <Tooltip 
                        contentStyle={themeMode === 'dark' ? powerBITooltipDarkStyles : powerBITooltipStyles}
                        formatter={(value, name) => [value, 'Asistencias']}
                      />
                      <Bar 
                        dataKey="cantidad" 
                ...
                <Box>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={eventAttendanceMetrics.byMonth.length > 0 ? eventAttendanceMetrics.byMonth : [
                      { month: '2024-01', events: 0, confirmations: 0, attendance_rate: 0 },
                      { month: '2024-02', events: 0, confirmations: 0, attendance_rate: 0 },
                      { month: '2024-03', events: 0, confirmations: 0, attendance_rate: 0 },
                      { month: '2024-04', events: 0, confirmations: 0, attendance_rate: 0 },
                      { month: '2024-05', events: 0, confirmations: 0, attendance_rate: 0 },
                      { month: '2024-06', events: 0, confirmations: 0, attendance_rate: 0 }
                    ]}>
                      <defs>
                        ...
                        <linearGradient id="confirmationsGradient" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="#22c55e" stopOpacity={0.8}/>
                          <stop offset="95%" stopColor="#22c55e" stopOpacity={0.1}/>
                        </linearGradient>
                      </defs>
                      ...
                      <Tooltip 
                        contentStyle={themeMode === 'dark' ? powerBITooltipDarkStyles : powerBITooltipStyles}
                        formatter={(value, name) => [
                          name === 'events' ? `${value} eventos` : 
                          name === 'confirmations' ? `${value} confirmaciones` : 
                          `${value}%`,
                          name === 'events' ? '📅 Eventos' : 
                          name === 'confirmations' ? '✅ Confirmaciones' : 
                          '📊 Tasa de Asistencia'
                        ]}
                      />
                      <Legend 
                      ...
                      <Line 
                        ...
                        stroke="#22c55e" 
                        strokeWidth={3}
                        fill="url(#confirmationsGradient)"
              ...
              {eventAttendanceMetrics.topEvents.length > 0 && (
                <Box sx={{ mt: 4 }}>
                  <Typography variant="h6" sx={{ 
                    mb: 3, 
                    fontWeight: 700, 
                  ...
                  <TableContainer component={Paper} sx={{ 
                    ...
                    <Table>
                      <TableHead>
                        <TableRow sx={{ bgcolor: themeMode === 'dark' ? '#475569' : '#f8fafc' }}>
                          <TableCell sx={{ fontWeight: 700, color: themeMode === 'dark' ? '#f1f5f9' : '#1e293b' }}>
                            Evento
                          </TableCell>
                          <TableCell sx={{ fontWeight: 700, color: themeMode === 'dark' ? '#f1f5f9' : '#1e293b' }}>
                            Tipo
                      ...
                      <TableBody>
                        {eventAttendanceMetrics.topEvents.slice(0, 5).map((event, index) => (
                          <TableRow 
                            key={event.id}
                            sx={{ 
                              '&:nth-of-type(odd)': { 
                          ...
                          >
                            <TableCell sx={{ color: themeMode === 'dark' ? '#f1f5f9' : '#1e293b' }}>
                              {event.title}
                            </TableCell>
                            <TableCell>
                              <Chip 
                            ...
                            <TableCell>
                              <Chip 
                                label={`${event.confirmation_rate}%`} 
                                size="small" 
                                color={event.confirmation_rate >= 80 ? 'success' : event.confirmation_rate >= 60 ? 'warning' : 'error'}
                                sx={{ fontSize: '0.75rem' }}
                              />
                            </TableCell>
                          </TableRow>
