...
# VISTA ELIMINADA - DUPLICADA CON custom_admin/views.py 
...
def companies_list_admin(request):
    """Lista de empresas para administradores - SOLO RATING - DATOS REALES"""
    try:
        print("🚀🚀🚀 [COMPANIES LIST ADMIN - LIMPIA] Vista iniciada 🚀🚀🚀")
        print(f"🔍 [COMPANIES LIST ADMIN - LIMPIA] Usuario: {request.user.email}")
        print(f"🔍 [COMPANIES LIST ADMIN - LIMPIA] Rol: {request.user.role}")

        # Verificar que sea admin
        if not hasattr(request.user, 'role') or request.user.role != 'admin':
            print("❌ [COMPANIES LIST ADMIN - LIMPIA] Usuario no es admin")
            return JsonResponse({'error': 'Acceso denegado'}, status=403)

        print("✅ [COMPANIES LIST ADMIN - LIMPIA] Usuario es admin, continuando...")
        ...
        print(f"🔍 [COMPANIES LIST ADMIN - LIMPIA] Parámetros: limit={limit}, offset={offset}, search='{search}', status='{status}'")

        # Query base
        queryset = Empresa.objects.select_related('user').all()
        print(f"🔍 [COMPANIES LIST ADMIN - LIMPIA] Query base creada, total empresas: {queryset.count()}")
        ...
        if search:
            ...
            print(f"🔍 [COMPANIES LIST ADMIN - LIMPIA] Filtro de búsqueda aplicado: '{search}'")

        if status:
            queryset = queryset.filter(status=status)
            print(f"🔍 [COMPANIES LIST ADMIN - LIMPIA] Filtro de estado aplicado: '{status}'")

        # Contar total
        total_count = queryset.count()
        print(f"🔍 [COMPANIES LIST ADMIN - LIMPIA] Total después de filtros: {total_count}")
        ...
        print(f"🔍 [COMPANIES LIST ADMIN - LIMPIA] Empresas a procesar: {len(companies)}")

        # ✅ SERIALIZER COMPLETAMENTE MANUAL - SOLO 19 CAMPOS - DATOS REALES
        companies_data = []
        for company in companies:
            print(f"\n🏢 [COMPANIES LIST ADMIN - LIMPIA] Procesando empresa: {company.company_name}")

            # ✅ RATING REAL DE LA BASE DE DATOS
            rating_valor = float(company.rating) if company.rating is not None else 0.0
            print(f"   - Rating de BD: {rating_valor} (tipo: {type(rating_valor)})")
            ...
            # ✅ DATOS COMPLETAMENTE MANUALES - SOLO 19 CAMPOS - DATOS REALES
            company_data = {
                'id': str(company.id),
                'user': str(company.user.id),
                'company_name': company.company_name,
                ...
                'size': company.size,
                'verified': company.verified,
                'rating': rating_valor,  # ✅ SOLO RATING - VALOR REAL DE LA BD
                'total_projects': company.total_projects,
                'projects_completed': company.projects_completed,
                'total_hours_offered': company.total_hours_offered,
                'created_at': company.created_at.isoformat(),
            ...
            print(f"   - Rating en company_data: {company_data.get('rating')} (tipo: {type(company_data.get('rating'))})")
            print(f"   - ¿Tiene rating? {'rating' in company_data}")
            print(f"   - Keys de company_data: {list(company_data.keys())}")
            print(f"   - Total de campos: {len(company_data.keys())}")

            companies_data.append(company_data)

        print(f"✅ [COMPANIES LIST ADMIN - LIMPIA] Datos serializados: {len(companies_data)} empresas")

        # 🔍 DEBUG: Ver la primera empresa para verificar los campos
        if companies_data:
            primera_empresa = companies_data[0]
            print(f"🔍 [COMPANIES LIST ADMIN - LIMPIA] Primera empresa serializada:")
            print(f"   - ID: {primera_empresa['id']}")
            print(f"   - Nombre: {primera_empresa['company_name']}")
            print(f"   - Rating: {primera_empresa['rating']}")
            print(f"   - Status: {primera_empresa['status']}")
            print(f"   - Tipo de rating: {type(primera_empresa['rating'])}")
            print(f"   - Rating es NaN? {primera_empresa['rating'] != primera_empresa['rating']}")
            print(f"   - Campos disponibles: {list(primera_empresa.keys())}")

        # 🔍 DEBUG: Ver la respuesta JSON a enviar
        response_data = {
            'results': companies_data,
            'count': total_count
        }

        print(f"🔍 [COMPANIES LIST ADMIN - LIMPIA] Respuesta JSON a enviar:")
        print(f"   - Tipo de results: {type(response_data['results'])}")
        print(f"   - Longitud de results: {len(response_data['results'])}")
        if response_data['results']:
            print(f"   - Primer elemento tipo: {type(response_data['results'][0])}")
            print(f"   - Keys del primer elemento: {list(response_data['results'][0].keys())}")
            print(f"   - Total de campos del primer elemento: {len(response_data['results'][0].keys())}")

        print("🚀🚀🚀 [COMPANIES LIST ADMIN - LIMPIA] Enviando respuesta JSON 🚀🚀🚀")
        return JsonResponse(response_data)

    except Exception as e:
        print(f"❌ [COMPANIES LIST ADMIN - LIMPIA] Error en la vista: {e}")
        import traceback
        traceback.print_exc()
        return JsonResponse({'error': str(e)}, status=500) 